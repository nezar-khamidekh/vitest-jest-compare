...
function simulateAsyncOperation(delay: number): Promise<string> {
...
function simulateFileRead(filename: string): Promise<string> {
  return new Promise((resolve) => {
    setTimeout(() => {
...
function simulateHttpRequest(url: string): Promise<{ status: number; data: any }> {
  return new Promise((resolve) => {
    setTimeout(() => {
      resolve({
        status: 200,
